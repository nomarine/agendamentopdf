...
$sql = "Select LEFT(start,5) AS start, LEFT(c_name, 40) AS c_name, SUBSTRING(c_phone, 3, 100) AS c_phone, date from appscheduler_bookings_services left join appscheduler_bookings on appscheduler_bookings_services.booking_id = appscheduler_bookings.id where date = '" . $dataatual . "' AND booking_status = 'confirmed' ORDER BY start;";
